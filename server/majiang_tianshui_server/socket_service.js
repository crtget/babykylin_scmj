...
exports.start = function (config, mgr) {
	...
	io.sockets.on('connection', function (socket) {
		socket.on('login', function (data) {
			...
			roomInfo.seats[seatIndex].ip = socket.handshake.headers['x-forwarded-for'] || socket.handshake.address;
		...
		socket.on('begin', function (data) {
			...
			var roomId = roomMgr.getUserRoom(userId);
			if (roomId == null) {
				console.log("dissolve no roomid !!");
				return;
			}
			...
			var ret = roomMgr.isAllReady(roomId);
			if (ret) {
				//处理游戏开始
				socket.gameMgr.begin(roomId);
			}

			userMgr.sendMsg(userId, 'user_begin_push', { ret: ret });
		});
		...
		socket.on('zuolapao', function (data) {
			if (socket.userId == null) {
				return;
			}
			if (data == null) {
			...
			socket.gameMgr.zuoLaPao(socket.userId, data);
		});
		...
		//扣牌
		socket.on('koupai', function (data) {
			if (socket.userId == null) {
				return;
			}
			socket.gameMgr.kouPai(socket.userId, data);
		});
		socket.on('yaoshaizi', function (data) {
			var shaizi = 0;
			while (shaizi < 2) {
				shaizi = Math.ceil(Math.random() * 12);
			}
			userMgr.broacastInRoom('yaoshaizi', { shaizi: shaizi }, socket.userId, true);
		});
		//真换牌
		socket.on('zhenhuanpai', function (data) {
			if (socket.userId == null) {
				return;
			}
			if (data == null) {
			...
			var pai = data;
			socket.gameMgr.zhenhuanpai(socket.userId, pai);
		});
		//出牌
		...
		socket.on('chi', function (data) {
			...
			var num = data;
			socket.gameMgr.chi(socket.userId, num);
		});
		...
		//听
		socket.on('ting', function (data) {
			if (socket.userId == null) {
				return;
			}
			socket.gameMgr.ting(socket.userId);
		});
		...
		socket.on('dissolve_agree', function (data) {
			...
			if (ret != null) {
				...
				var renshu = socket.gameMgr.getRenshu(roomId);
				console.log("tongyinum=====" + doAllAgree)
				if (doAllAgree >= renshu - 1) {
					socket.gameMgr.doDissolve(roomId);
					console.log("dissolve_all_agree!")
				}
			}
		});
		...
		socket.on('update_position', function (data) {
			...
			roomMgr.setPosition(userId, lat, lng);
			var posData = roomMgr.getAllPosition(userId);
			userMgr.broacastInRoom('update_position_push', posData, userId, true);
		});
	});
